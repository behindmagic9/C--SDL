...
struct Car {
    Car(SDL_Renderer* render) 
        : renderer(render), x(0), y(SCREEN_HEIGHT / 2), isJumping(false), jumpVelocity(0.0f), 
...
int main(int argc, char* argv[]) {
    ...
    while (GameIsRunning) {
        while (SDL_PollEvent(&event)) {
            ...
            if (event.type == SDL_KEYDOWN) {
                if (gameState == START && event.key.keysym.sym == SDLK_RETURN) {
                    ...
                    car.Jump();
                } else if (gameState == GAMEOVER && event.key.keysym.sym == SDLK_RETURN) {
                    gameState = PLAYING; // Restart the game
                    // Reset game entities here (e.g., car position, tile positions, etc.)
                }
            }
        }
        ...
        if (gameState == START) {
            // Render the start screen (e.g., press enter to start)
            SDL_SetRenderDrawColor(renderer, 255, 255, 255, 255);
            // You can use SDL_ttf for text rendering here
            ...
            for (auto& tiles : tiles_vector_bottom) {
                ...
                if (tiles.Collide(car.getRect())) {
                    ...
                    std::cout << "Collided with bottom tile" << std::endl;
                }
            }
            ...
            for (auto& tiles : tiles_vector_top) {
                ...
                if (tiles.Collide(car.getRect())) {
                    gameState = GAMEOVER; // Trigger game over
                    std::cout << "Collided with top tile" << std::endl;
                }
            }
            ...
            // Render game over screen (e.g., press enter to restart)
            SDL_SetRenderDrawColor(renderer, 255, 0, 0, 255);
            // Again, you can render text using SDL_ttf here
            SDL_RenderPresent(renderer);
        }
