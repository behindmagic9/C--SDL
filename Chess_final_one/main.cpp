...
public:
    ...
    // this be basically rturn all the mpves of the ai color piece on the board
    ...
    // this function be basciallu evalutate the board
    ...
    // assigning and return the value to certain peice of the chess
    ...
    // giving the color of the opposite player
    ...
    Piece movePieceWithCapture(int srcX, int srcY, int destX, int destY)
    {
        ...
        board[srcY][srcX] = Piece(Troops::None, Color::None); // Empty the source tile
    ...
    bool isgameOver(Color currentTurnColor)
    {
        ...
        // If no valid moves and the king is in check, it's checkmate
        if (IsKingCheck(kx, ky, currentTurnColor))
        {
    ...
    int minimax(int depth, bool isMaximizingPlayer, int alpha, int beta, Color aiColor)
    {
        ...
        {
            return evaluateBoard(aiColor); // Evaluate the board if we've hit max depth or a terminal state.
        }

        std::vector<std::tuple<int, int, int, int>> allMoves = generateAllMoves(isMaximizingPlayer ? aiColor : getOppositeColor(aiColor));
    ...
    std::pair<std::pair<int, int>, std::pair<int, int>> makeAIMove(Color aiColor)
    {
        int bestValue = -10000;
        std::tuple<int, int, int, int> bestMove;
        SDL_Delay(2000);

        // Generate all possible moves for the AI's color
        ...
        for (auto move : allMoves)
        {
            ...
            // Call minimax to evaluate the move (with alpha-beta pruning)
            int moveValue = minimax(3, false, -10000, 10000, aiColor); // Depth of 3
        ...
        // Extract the best move (srcX, srcY) -> (destX, destY)
        int srcX, srcY, destX, destY;
        std::tie(srcX, srcY, destX, destY) = bestMove;

        // Return the best move as a pair of coordinates
                            ...
                            if (moveSuccess)
                            {
                                ...
                                else
                                {
                                    ...
                                    if (currentPlayerColor == aiColor)
                                    {
                                        ...
                                        // SDL_Delay(2000); // 1-second delay before AI makes its move
                                        // Generate and make AI move
