...
struct Boss
{
    ...
    Boss(int x, int y) : bossRect{x, y, 150, 600}, health(20), active(true) {}

    void shoot(std::vector<Fire> &fires, const SDL_Rect &playerRect)
    {
        Fire newFire;
        newFire.fireRect.x = bossRect.x + bossRect.w / 2;                                        // Center of the boss
        newFire.fireRect.y = bossRect.y + bossRect.h;                                            // Just below the boss
        newFire.fireRect.w = 20;                                                                 // Width of fireball
        newFire.fireRect.h = 10;                                                                 // Height of fireball
        newFire.fireSpeed = 5.0f;                                                                // Speed of the fireball
        newFire.direction = (playerRect.x + playerRect.w / 2 - newFire.fireRect.x) > 0 ? 1 : -1; // Determine direction
        newFire.active = true;
...
int main()
{
    ...
    // enemies.emplace_back(500,500,80,80);
    ...
    Boss boss(640, 50);
    bool bossFightActive = false; // Flag to check if the boss fight is active
    ...
    while (gameIsRunning)
    {
        ...
        if (gamestate == PLAYING)
        {
            ...
            if (!bossFightActive && player.score >= 100)
            {
            ...
            {
                boss.shoot(fires, player.Player_rect); // Boss shoots at player

                // Check for collision with boss and player fireballs
                for (auto &fire : fires)
                {
                    if (fire.active && SDL_HasIntersection(&fire.fireRect, &boss.bossRect))
                    {
                        fire.active = false; // Fireball hits boss
                        boss.health -= 1;    // Decrease boss health
                        if (boss.health <= 0)
                        {
                            boss.active = false; // Boss defeated
                        }
                    }
                }
            }
            ...
            if (bossFightActive && boss.active)
            {
                SDL_RenderCopy(render, enemy_texture, nullptr, &boss.bossRect); // Render boss
            }
