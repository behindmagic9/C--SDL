...
int main()
{
    ...
    SDL_Surface *StartScreen_Surface = SDL_LoadBMP("images/startScreen.bmp");
    SDL_Surface *GameOver_Surface = SDL_LoadBMP("images/GameOver.bmp");
    ...
    if (!background_Space || !RightPerson || !fireSurface || !heart_surface || !dustSurface || !tile_surface || !enemy_surface  || !StartScreen_Surface || !GameOver_Surface)
    {
    ...
    SDL_Texture *StartScreen_texture = SDL_CreateTextureFromSurface(render, StartScreen_Surface);
    SDL_Texture *GameOver_texture = SDL_CreateTextureFromSurface(render, GameOver_Surface);
...
// background track
    ...
    while (gameIsRunning)
    {
        ...
        else if (gamestate == Menu)
        {
            ...
            SDL_RenderCopy(render, StartScreen_texture, nullptr, &start_rect);
            // SDL_RenderCopy
        }
        else if (gamestate == GAMEOVER)
        {
            SDL_RenderClear(render);
            SDL_SetRenderDrawColor(render, 255, 0, 0, 255);
            SDL_Rect gameover_rect {0,0,1280,720};
            SDL_RenderCopy(render, GameOver_texture, nullptr, &gameover_rect);
            std::string stringscore = std::to_string(player.score);
            std::string stringHealth = std::to_string(player.health);
            RenderText(render, "Your HighScore is : " + stringscore, 350, 350, font);
            RenderText(render, "Press 'Q' to Quit the game !!", 500, 500, font);
        }
